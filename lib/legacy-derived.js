...
function extractFirstReplyTime(ticket) {
  ...
  // Was `.business || .calendar`, preserved for parity with the old app.
  // That parity was the bug: the SLA policy measures on calendar.
...
function extractResolutionTime(ticket) {
  // Same calendar-first rule as first reply.
  if (ticket.metric_set) {
    // Calendar, not business: every target in the Zendesk SLA policy is
    // business_hours: false, because coverage is continuous. `??` not `||` —
    // a calendar time of zero is a real value, and `||` would fall through to
